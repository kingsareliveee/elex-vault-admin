...
export const Dashboard: React.FC = () => {
  ...
  const approvedCount = resources.filter(r => r.isApproved === true).length;
  // Rejected counts computed from audit logs since rejected rows are deleted from DB
  const rejectedCount = auditLogs.filter(log => log.action.includes('REJECTED')).length;
  
  const uniqueContributors = Array.from(new Set(resources.map(r => r.contributorName))).length;
  ...
  const contributorStats = resources.reduce((acc: any, curr) => {
    const name = curr.contributorName;
    if (!acc[name]) {
      acc[name] = { name, total: 0, approved: 0, pending: 0 };
    }
    acc[name].total += 1;
    if (curr.isApproved) acc[name].approved += 1;
    if (!curr.isApproved) acc[name].pending += 1;
    return acc;
  }, {});
  ...
  const recentPending = resources.filter(r => r.isApproved === false).slice(0, 3);
    ...
    <div className="p-6 space-y-6 font-sans select-none">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
        ...
        <div className="xl:col-span-2 flex flex-col bg-obsidian-900 border border-zinc-800 rounded-xl overflow-hidden shadow-sm">
          ...
          <div className="flex-1 p-5 text-xs space-y-4 max-h-[460px] overflow-y-auto bg-obsidian-950/10">
            {auditLogs.length === 0 ? (
            ...
            <div className="p-4 space-y-3">
              {recentPending.length === 0 ? (
                <div className="text-center py-6 text-zinc-500 text-xs">
                  Review queue clear. No pending items.
                </div>
              ) : (
